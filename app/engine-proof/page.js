...
function DynamicScene({result,stepIndex=0}){
 ...
 const steps=Array.isArray(result?.experience?.steps)?result.experience.steps:[];
 const safeStep=Math.max(0,Math.min(Math.max(0,steps.length-1),Number(stepIndex)||0));
...
export default function Page(){
 ...
 const dynamicSteps=Array.isArray(engineResult?.experience?.steps)?engineResult.experience.steps:[];
 const currentDynamicStep=dynamicSteps[Math.max(0,Math.min(dynamicStep,Math.max(0,dynamicSteps.length-1)))]||null;
 const concepts=[
 ...
 async function runEngine(e){
  ...
  try{
   const response=await fetch("/api/engine",{
    ...
    body:JSON.stringify({question:q,context:{scene:active,lastQuestion,audience:"عام",previous:engineResult&&!engineResult.error?{title:engineResult.topic,summary:engineResult.answer,domain:engineResult.domain,topic:engineResult.topic,truthAnchors:engineResult.experience?.truthAnchors||[],causalRelations:(engineResult.experience?.sceneGraph?.edges||[]).filter(e=>e?.causal).slice(0,8).map(e=>({from:e.from,to:e.to,relation:e.relation,label:e.label})),sceneGraph:{nodes:(engineResult.experience?.sceneGraph?.nodes||[]).slice(0,8).map(n=>({id:n.id,label:n.label}))}}:null}})
   });
 ...
 return <main className="heartPlatform" dir="rtl">
  <section className="heartScreen" aria-label="رحلة القلب">
   ...
   <div className={stageClass} style={stageStyle}>
     {dynamicMode?<DynamicScene result={engineResult} stepIndex={dynamicStep}/>:<HeartScene kind={["overview","inside","chambers","valves","flow","lungs","body","coronary","electric","whole"][active]}/>}
     {!dynamicMode&&<VisualGuide index={active}/>}
     <div className="stageCaption"><b>{dynamicMode?(currentDynamicStep?.title||engineResult?.topic||"شرح بصري"):scene.title}</b><span>{dynamicMode?(currentDynamicStep?.outcome||currentDynamicStep?.why||renderPlan?.target||engineResult?.explanation):scene.sub}</span></div>{dynamicMode&&dynamicSteps.length>1&&<div className="dynamicStepControls"><button onClick={()=>setDynamicStep(v=>Math.max(0,v-1))} disabled={dynamicStep<=0}>السابق</button><span>{Math.min(dynamicStep+1,dynamicSteps.length)} / {dynamicSteps.length}</span><button onClick={()=>setDynamicStep(v=>Math.min(dynamicSteps.length-1,v+1))} disabled={dynamicStep>=dynamicSteps.length-1}>التالي</button></div>}
   </div>

   <form className="engineAsk" onSubmit={runEngine}><input value={engineQuestion} onChange={e=>setEngineQuestion(e.target.value)} placeholder="جرّب: لماذا لا يرجع الدم؟" disabled={engineLoading}/><button disabled={engineLoading}>{engineLoading?"أفهم…":"نفّذ"}</button></form>{lastQuestion&&<div className="engineContext">Context: {lastQuestion}</div>}{engineResult&&<div className={"engineResult "+(engineResult.error?"isError":"")}><b>{engineResult.error?"لم أُنفّذ تخمينًا":"النتيجة"}</b><span>{engineResult.error||engineResult.answer}</span>{!engineResult.error&&<><small>{engineResult.explanation}</small><em>{engineResult.visualPlan?.operations?.join(" → ")}</em><i>{engineResult.verification?.status||"unverified"} · {engineResult.confidence||"low"}{engineProvider?" · "+engineProvider:""}{engineResult.verification?.sources?.length?" · sources "+engineResult.verification.sources.length:""}</i>{engineResult.verification?.sources?.length>0&&<div className="sourceChips">{engineResult.verification.sources.slice(0,2).map((s,i)=><a key={i} href={s.url} target="_blank" rel="noreferrer">{s.title}</a>)}</div>}</>}</div>}
