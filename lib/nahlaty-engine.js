...
export function sanitizeEngineResult(result, local) {
  ...
  const scene=Number.isInteger(result?.scene)?Math.min(9,Math.max(0,result.scene)):(local?.scene??0);
  return {
    domain: String(result?.domain||local?.domain||"general").slice(0,60),
    topic: String(result?.topic||local?.topic||"unknown").slice(0,80),
    answer: String(result?.answer||local?.answer||"").slice(0,900),
    explanation: String(result?.explanation||local?.explanation||"").slice(0,1200),
    scene,
    confidence: ["high","medium","low"].includes(result?.confidence)?result.confidence:(local?.confidence||"low"),
    needsVerification: Boolean(result?.needsVerification) && !local,
    visualPlan: {
