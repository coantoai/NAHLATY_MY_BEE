const packs=[
 ...
 },
 {
  ...
  keywords:["خلية شمسية","الخلية الشمسية","خلايا شمسية","لوح شمسي","ألواح شمسية","طاقة شمسية","solar cell","photovoltaic","pv cell","solar panel"],
 ...
 },
 {
  ...
  keywords:["محرك احتراق","محرك الاحتراق الداخلي","الاحتراق الداخلي","محرك السيارة","محرك بنزين","بستم","مكبس","احتراق داخلي","internal combustion","car engine","piston engine","gasoline engine"],
...
export function curatedKnowledgeResultById(id){
 const p=packs.find(pack=>pack.id===String(id||""));
 if(!p)return null;
 return {
  domain:p.domain,
  topic:p.id,
  ...
  verification:{
   ...
   source:"NAHLATY curated knowledge pack v1",
   sources:p.sources,
   note:"Context preserved inside the same sourced knowledge world."
  },
  experience:toExperience(p)
 };
}
